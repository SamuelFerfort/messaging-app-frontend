...
const MessagesWindow = ({ messages, messagesLoading, messagesError }) => {
  ...
  if (messagesLoading) {
   return <main className="flex-grow flex flex-col chat items-center justify-center h-full">
        <Loading  size={50}/>
    </main>
  }
  if (messagesError)
      ...
      {messages.map((m) => (
        <div
          ...
          className={`flex  items-center gap-1 ${
            m.senderId === user.id ? "ml-auto  flex-row-reverse " : "mr-auto "
          }`}
        >
          {m.sender.avatar ? (
            <img
          ...
          {m.type === "TEXT" ? (
            <div
              className={`py-2 px-3 my-1 rounded-lg chat-message flex gap-2 items-center ${
                m.senderId === user.id ? " bg-green-200 " : " bg-slate-100"
              }  `}
              key={m.id}
            >
              <span>{m.content}</span>
